...
{
    particleArray = new mechaEngine::Particle[particleCount];
...
void MassAggregateApplication::display()
{
    ...
    mechaEngine::ParticleWorld::Particles &particles = world.getParticles();
    for (mechaEngine::ParticleWorld::Particles::iterator p = particles.begin();
        p != particles.end();
        p++)
    {
        mechaEngine::Particle *particle = *p;
        const mechaEngine::Vector3 &pos = particle->getPosition();
        glPushMatrix();
